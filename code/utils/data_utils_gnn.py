...
from utils.data_utils_txt import calc_elapsed_time
...
class Prepare_GNN_Dataset():
    ...
    def prepare_gnn_training(self, verbose):
        if verbose:
        ...
        print('No. of nodes in graph = ', new_num_nodes)
        ...
        print("No.of test instances = ",
              new_num_nodes - self.data.train_mask.sum().item() - self.data.val_mask.sum().item())
