...
def log_tensorboard(config, writer, model, epoch, iters, total_iters, loss, f1, prec, recall, acc, lr=0, thresh=0,
                    loss_only=True, val=False):
...
def print_stats(config, epoch, train_loss, train_acc, train_f1, train_f1_macro, train_prec, train_recall, val_loss,
                val_acc, val_f1, val_f1_macro, val_precision, val_recall, start, lr):
    end = time.time()
    ...
    print("\nEpoch: {}/{},  \
          ...
          .format(epoch, config['max_epoch'], train_loss, train_acc, train_prec, train_recall, train_f1, train_f1_macro,
                  val_loss, val_acc,
...
def print_transformer_results(val_stats, test_stats, val_result, test_result):
