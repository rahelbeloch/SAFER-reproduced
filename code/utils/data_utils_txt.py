...
class Prepare_Dataset():
    ...
    def prepare_glove_training(self, verbose=True):
        ...
        train, val, test = FakeNews_dataset.splits(data_dir, train=os.path.join('train.tsv'),
                                                   ...
                                                   format='tsv', fields=[('text', FakeNews_dataset.TEXT),
                                                                         ('label', FakeNews_dataset.LABEL),
                                                                         ('id', FakeNews_dataset.ID)])
        ...
        FakeNews_dataset.TEXT.vocab.vectors[FakeNews_dataset.TEXT.vocab.stoi['<unk>']] = torch.mean(
            FakeNews_dataset.TEXT.vocab.vectors, dim=0)

        train_loader, dev_loader, test_loader = BucketIterator.splits((train, val, test),
    ...
    def prepare_HAN_elmo_training(config):
        train_data, val_data, test_data = Dataset_Helper_HAN.get_dataset(config,
    ...
    def prepare_lr_training(config, seed, fold=None):
        ...
        train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=config['batch_size'],
                                                   collate_fn=collate_for_lr, shuffle=True)
...
class FakeNews_dataset(TabularDataset):
    TEXT = Field(sequential=True, batch_first=True, lower=True, use_vocab=True, tokenize=clean_string,
...
class FakeNews_dataset_CNN(FakeNews_dataset):
    TEXT = Field(sequential=True, batch_first=True, lower=True, use_vocab=True, tokenize=clean_string_stop_words_remove,
                 include_lengths=True)
...
class HAN_Dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        result_embeddings = []
        doc_elmo_ids = batch_to_ids(body_tokens).squeeze(
            0)  # bach_to_ids returns a tensor that is (B x num_words x 50) = (1 x num_words x 50) here
...
class DataLoader_Helper_HAN():
    @classmethod
    def create_dataloaders(cls, train_dataset: data.Dataset = None, validation_dataset: data.Dataset = None,
                           test_dataset: data.Dataset = None, batch_size: int = 32, shuffle: bool = True):
        '''
...
class HAN_Batch():
    ...
    def just_pad_batch(self):
        ...
        for batch in range(batch_size):
            for num_sent in range(self._num_of_sent[batch]):
                ...
                i += 1
        return torch.Tensor(padded_sequences), torch.Tensor(self._targets), torch.LongTensor(
            self._num_of_sent), torch.LongTensor(padded_lengths)
    ...
    def pad_batch(self):
        ...
        return torch.Tensor(padded_sequences), torch.Tensor(self._targets), torch.LongTensor(
            self._num_of_sent), torch.LongTensor(concat_lengths)
    ...
    def pad_and_sort_batch(self):
        ...
        for i, l in enumerate(concat_lengths):
            padded_sequences[i, :l, :] = concat_sequences[i].squeeze(1)
        return self._sort_batch(torch.Tensor(padded_sequences), torch.Tensor(self._targets),
                                torch.LongTensor(self._num_of_sent), torch.LongTensor(concat_lengths))
...
class LR_Dataset(data.Dataset):
    def __init__(self, config, split, seed):
        ...
        if config['data_name'] in ['gossipcop', 'politifact']:
            ...
            self.doc_embeds_file_gnn = os.path.join(self.base_dir, 'cached_embeds',
                                                    'doc_embeds_graph_lr_wt3_30_30_{}_{}_{}.pt'.format(split, seed,
                                                                                                       config[
                                                                                                           'model_name']))
            # self.doc_embeds_file_text = os.path.join(self.base_dir, 'cached_embeds', 'doc_embeds_roberta_lr_{}.pt'.format(split))
            self.doc_embeds_file_text = os.path.join(self.base_dir, 'cached_embeds',
                                                     'doc_embeds_cnn_21_{}.pt'.format(split))
            ...
            self.doc_embeds_file_gnn = os.path.join(self.base_dir, 'cached_embeds',
                                                    'doc_embeds_graph_poinc_wt3_attn_lr_{}_{}_{}.pt'.format(split, seed,
                                                                                                            config[
                                                                                                                'model_name']))
            ...
            self.doc_embeds_file_text = os.path.join(self.base_dir, 'cached_embeds',
                                                     'doc_embeds_cnn_42_{}.pt'.format(split))
        ...
        if config['data_name'] in ['gossipcop', 'politifact']:
            self.split_docs_labels_file = os.path.join(self.base_dir, 'cached_embeds',
                                                       'docs_labels_lr_30_30_{}.json'.format(split))
        else:
            self.split_docs_labels_file = os.path.join(self.base_dir, 'cached_embeds',
                                                       'docs_labels_lr_{}.json'.format(split))
