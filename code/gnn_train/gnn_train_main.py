...
from utils.utils import evaluation_measures, log_tensorboard
...
class Graph_Net_Main():
    ...
    def init_training_params(self):
        ...
        if self.config['model_name'] in ['HGCN', 'HNN']:
            ...
            self.model = Graph_Net(self.config).to(device) if not self.config['model_name'].startswith(
                'r') else Relational_GNN(self.config).to(device)

        if self.config['optimizer'] == 'Adam':
            self.optimizer = torch.optim.AdamW(self.model.parameters(), lr=self.config['lr'],
                                               weight_decay=self.config['weight_decay'])
        elif self.config['optimizer'] == 'RAdam':
            self.optimizer = RiemannianAdam(self.model.parameters(), lr=self.config['lr'],
                                            weight_decay=self.config['weight_decay'])
        ...
        with torch.no_grad():
            if not self.config['full_graph']:
                for iters, eval_data in enumerate(self.config['loader']):
                    ...
                    preds = preds.squeeze(1).to(device) if self.config['loss_func'] == 'bce_logits' else preds.to(
                        device)
                    loss = self.criterion(preds, labels.to(device))
                    self.criterion(preds, labels.to(device))
                    eval_loss.append(loss.detach().item())
                    if self.config['loss_func'] == 'ce':
                        preds = F.softmax(preds, dim=1)
                        preds = torch.argmax(preds, dim=1)
                    else:
                        preds = (F.sigmoid(preds) > 0.5).type(torch.FloatTensor) if self.config[
                                                                                        'loss_func'] == 'bce_logits' else (
                                preds > 0.5).type(torch.FloatTensor)
                ...
                if self.config['model_name'] in ['rgcn', 'rgat', 'rsage']:
                    preds, node_drop_mask, _ = self.model(self.config['data'].x.to(device),
                                                          self.config['data'].edge_index.to(device),
                                                          self.config['data'].edge_attr.to(device))
                elif self.config['model_name'] in ['HGCN', 'HNN']:
                    preds = self.model.encode(self.config['data'].x.to(device),
                                              self.config['data'].edge_index.to(device))
                else:
                    preds, node_drop_mask, _ = self.model(self.config['data'].x.to(device),
                                                          self.config['data'].edge_index.to(device))
            ...
            eval_f1, eval_macro_f1, eval_recall, eval_precision, eval_accuracy = evaluation_measures(self.config,
                                                                                                     np.array(
                                                                                                         preds_list),
                                                                                                     np.array(
                                                                                                         labels_list))
    ...
    def check_early_stopping(self):
        ...
        new_best = self.this_metric > self.current_best if self.config[
                                                               'optimze_for'] == 'f1' else self.this_metric < self.current_best
        if new_best:
        ...
        diff = self.this_metric - self.current_best if self.config[
                                                           'optimze_for'] == 'f1' else self.current_best - self.this_metric
        if diff < 1e-3:
            self.not_improved += 1
    ...
    def train_epoch_step(self):
        ...
        self.train_f1, self.train_macro_f1, self.train_recall, self.train_precision, self.train_accuracy = evaluation_measures(
            self.config, np.array(self.preds_list), np.array(self.labels_list))

        log_tensorboard(self.config, self.config['writer'], self.model, self.epoch, self.iters, self.total_iters,
                        self.train_loss, self.train_f1, self.train_precision, self.train_recall, self.train_accuracy,
                        lr[0], self.threshold, loss_only=False, val=False)

        # Evaluate on dev set
        self.eval_f1, self.eval_macro_f1, self.eval_precision, self.eval_recall, self.eval_accuracy, self.eval_loss = self.eval_gcn()

        # print stats
        print_stats(self.config, self.epoch, self.train_loss, self.train_accuracy, self.train_f1, self.train_macro_f1,
                    self.train_precision, self.train_recall,
                    self.eval_loss, self.eval_accuracy, self.eval_f1, self.eval_macro_f1, self.eval_precision,
                    self.eval_recall, self.start, lr[0])

        # log validation stats in tensorboard
        log_tensorboard(self.config, self.config['writer'], self.model, self.epoch, self.iters, self.total_iters,
                        self.eval_loss, self.eval_f1, self.eval_precision, self.eval_recall, self.eval_accuracy, lr[0],
                        self.threshold, loss_only=False, val=True)
    ...
    def train_iters_step(self):
        ...
        self.batch_labels = self.data.y[self.data.train_mask == 1].long() if self.config['data_name'] == 'pheme' else \
            self.data.y[self.data.train_mask == 1].float()
        self.loss = self.criterion(self.preds.to(device).squeeze(1), self.batch_labels.to(device)) if self.config[
                                                                                                          'loss_func'] == 'bce_logits' else self.criterion(
            self.preds.to(device), self.batch_labels.to(device))
        ...
        if self.iters % self.config['log_every'] == 0:
            # Loss only
            log_tensorboard(self.config, self.config['writer'], self.model, self.epoch, self.iters, self.total_iters,
                            self.train_loss, self.train_f1, self.train_precision, self.train_recall,
                            self.train_accuracy, loss_only=True, val=False)
    ...
    def train_main(self, cache):
            ...
            for self.epoch in range(self.start_epoch, self.config['max_epoch'] + 1):
                for self.iters, self.data in enumerate(self.config['loader']):
                    ...
                    if self.config['model_name'] in ['HGCN', 'HNN']:
                        ...
                        self.preds, self.node_drop_mask, _ = self.model(data_x.to(device),
        ...
        if self.terminate_training:
            print(
                "\n" + "-" * 100 + "\nTraining terminated early because the Validation loss did not improve for   {}   epochs".format(
