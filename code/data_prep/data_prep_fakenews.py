...
def prepare_data_corpus(new_dir=os.path.join('../data', 'base_data')):
    ...
    for data in dataset:
        ...
        for label in labels:
            ...
            with open(final_data_file, 'w+', encoding='utf-8') as csv_file:
                ...
                for file in all_files:
                    with open(file, 'r') as f:
                        ...
                        if data == 'politifact':
                            ...
                            # ID = file.split('\\')[-1]
                            # ID = ID.split('.')[0]
                            ID = file.split('gossipcop-')[1].replace('/news content.json', '')
                            doc2id[str(ID)] = c
                            c += 1
...
def create_data_splits(max_len=5000):
    ...
    for dataset in datasets:
        ...
        with open(src_dir, encoding='utf-8') as data:
            ...
            for row in reader:
                if isinstance(row['text'], str) and len(row['text']) > 25:
                    ...
                    doc_id.append(str(row['id'].replace('/news content', '')))
                else:
                    removed += 1
        ...
        doc_splits_file = os.path.join('../data/complete_data', dataset, 'doc_splits.json')
