...
class GNN_PreProcess():
    ...
    def build_vocab(self, vocab_file, dataset, train_docs, doc2id):
        ...
        print("\nBuilding vocabulary...")
        if dataset in ['gossipcop', 'politifact']:
            labels = ['fake', 'real']
            for label in labels:
                src_doc_dir = os.path.join(self.data_dir, 'base_data', dataset, label)
                for src_file_path in Path(src_doc_dir).glob('*'):

                    if not src_file_path.name.startswith('gossipcop'):
                        continue

                    doc_name = str(src_file_path.name.split('-')[1])

                    if doc_name not in train_docs or doc_name not in doc2id:
                        continue

                    with open(src_file_path / 'news content.json', 'r') as f:
                ...
                with open(vocab_file, 'w+') as v:
                    json.dump(vocab, v)
        elif dataset in ['HealthStory', 'HelathRelease']:
