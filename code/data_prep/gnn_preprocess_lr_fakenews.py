...
from gnn_preprocess_main import GNN_PreProcess
...
class GNN_PreProcess_news(GNN_PreProcess):
    ...
    def create_aggregate_folder(self):
        ...
        for dataset in self.datasets:
            ...
            for user_context in user_contexts:
                ...
                src_dir = os.path.join(self.data_dir, 'base_data', dataset, user_context)
                ...
                if user_context == 'tweets':
                    ...
                    for count, src_file_path in enumerate(Path(src_dir).glob('*')):
                        doc = src_file_path.name.split('.')[0]
    ...
    def create_doc_user_splits(self):
        ...
        for dataset in self.datasets:
            ...
            for count, src_file_path in enumerate(Path(src_dir).glob('*')):
                doc_key = src_file_path.name.split('.')[0]
    ...
    def create_dicts(self):
        ...
        for dataset in self.datasets:
            ...
            train_users = [u for u in train_users if u]
            val_users = [u for u in val_users if u]
            test_users = [u for u in test_users if u]
                ...
                for count, src_file_path in enumerate(Path(src_dir).glob('*')):
                    ...
                    if not os.path.isfile(dest_file_path):
                        if int(user_id) in done_users:
                            temp = set()
                            followers = src_file['followers'] if user_context == 'user_followers' else src_file[
                                'following']
    ...
    def create_adj_matrix(self):
        ...
        for dataset in self.datasets:
            ...
            for user_context in user_contexts:
                ...
                for count, src_file_path in enumerate(Path(src_dir2).glob('*')):
                    src_file = json.load(open(src_file_path, 'r'))
                    user_id = int(src_file['user_id'])
            ...
            self.save_adj_matrix(dataset, adj_matrix, edge_type)
    ...
    def create_feat_matrix(self, binary=True):
        ...
        for dataset in self.datasets:
            ...
            for label in labels:
                ...
                for i, file_dir in enumerate(Path(src_doc_dir).glob('*')):

                    if not file_dir.name.startswith('gossipcop'):
                        continue

                    doc_name = str(file_dir.name.split('-')[1])
                    if doc_name in split_docs and doc_name in doc2id and doc_name not in test_docs:
                        # feat_matrix[doc2id[str(doc_name)], :] = np.random.random(len(vocab)) > 0.99

                        doc_file = file_dir / 'news content.json'
            ...
            for count, file_path in enumerate(Path(src_dir).glob('*')):

                if not file_path.name.startswith('gossipcop'):
                    continue

                users = json.load(open(file_path, 'r'))['users']
                doc_name = str(file_path.name.split('-')[1])

                # if str(doc_key) in train_docs:

                # Each user of this doc has its features as the features of the doc
                if (doc_name in split_docs) and doc_name in doc2id:
                    for user in users:
                        if str(user) in user2id:
                            feat_matrix[user2id[str(user)], :] += feat_matrix[doc2id[doc_name], :]

                else:
                    if doc_name in doc2id:
                        real_file = os.path.join(self.data_dir, 'base_data', dataset, 'real', doc_name + '.json')
                        fake_file = os.path.join(self.data_dir, 'base_data', dataset, 'fake', doc_name + '.json')
                        ...
                        if os.path.isfile(this_file):
                            ...
                            with open(this_file, 'r') as f:
                                ...
                                text = nltk.word_tokenize(text)
                                for user in users:
                                    if int(user) in test_users and int(user) not in split_users and str(
                                            user) in user2id:
                                        vector = np.zeros(len(vocab))
                                        for token in text:
                                            if token in vocab.keys():
                ...
                # if count % print_iter == 0:
                #     print(" {} / {} done..".format(count + 1, total))
    ...
    def create_labels(self):
        ...
        for dataset in self.datasets:
            ...
            for user_context in user_contexts:
                data_dir = os.path.join(self.data_dir, 'base_data', dataset, user_context)
                label = 1 if user_context == 'fake' else 0

                for i, file_dir in enumerate(Path(data_dir).glob('*')):
                    if not file_dir.name.startswith('gossipcop'):
                        continue
                    doc_name = str(file_dir.name.split('-')[1])
                    if doc_name in split_docs:
                        self.doc2labels[doc_name] = label
...
if __name__ == '__main__':
    ...
    parser.add_argument('--create_aggregate_folder', type=bool, default=True,
